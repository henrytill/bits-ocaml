(* http://www.cs.cornell.edu/courses/cs3110/2012sp/lectures/lec29-fixpoints/lec29.html *)
...
(* Church numerals *)
let thunk x : church = Ch (fun _ _ -> x)
...
(* Church numerals *)
let pair x y = Ch (fun _ z -> if_zero z (thunk x) (thunk y))
...
let y t =
  let p (Fix f) x = t (f (Fix f)) x in
  p (Fix p)

let fact =
  let t_fact f x = if_zero x (thunk one) (Ch (fun _ _ -> mul x (f (sub1 x)))) in
  y t_fact

let fib =
  let t_fib f x = ite (lt x two) (thunk x) (Ch (fun _ _ -> add (f (sub1 x)) (f (sub x two)))) in
  y t_fib

let length =
  let t_length f x = ite (is_null x) (thunk zero) (Ch (fun _ _ -> add1 (f (snd x)))) in
  y t_length
